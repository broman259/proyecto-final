<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Reporte de Jugadores por Equipo') }}
        </h2>
    </x-slot>
                ...
                <div class="mx-auto">
                    <div class="relative flex flex-col w-full h-full text-slate-700 bg-white shadow-md rounded-xl bg-clip-border">
                        <div class="relative mx-4 mt-4 overflow-hidden text-slate-700 bg-white rounded-none bg-clip-border">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex flex-row gap-2 shrink-0">
                                    ...
                                    <form method="GET" action="{{ route('reportes.jugadoresEquipo') }}">
                                        <select name="equipo_id" onchange="this.form.submit()" class="pr-10 pl-4 py-2 mr-2 border border-gray-300 rounded-md shadow-sm">
                                            <option value="">Selecciona un equipo</option>
                                            @foreach($equipos as $equipo)
                                            <option value="{{ $equipo->id }}" {{ $equipoId == $equipo->id ? 'selected' : '' }}>
                                                {{ $equipo->nombre }}
                                            </option>
                                            @endforeach
                                        </select>
                                ...
                                <table class="w-full mt-4 text-left table-auto min-w-max">
                                    ...
                                    <tbody id="dataTable">
                                        <tr class="border-b border-slate-300">
                                            <td class="p-4 border-b bg-gray-100 border-slate-300">
                                            ...
                                            <td class="bg-gray-100 border-slate-300"></td>
                                            <td class="bg-gray-100 border-slate-300"></td>
                        ...
                        <div class="flex justify-center py-3">
                            {!! $equipos->appends(['search' => $search])->links() !!}
                        </div>
                    </div>
                </div>
            </div>
