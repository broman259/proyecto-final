    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                ...
                <form action="{{ route('punteos.update', $punteo->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mt-5 mx-7">
                        <div class="grid grid-cols-1">
                            ...
                            <select name="jornada_id" class="form-control select2 py-2 px-3 rounded-lg border-2 border-slate-300 mt-1 focus:outline-none focus:ring-2 focus:ring-slate-600 focus:border-transparent">
                                ...
                                <option value="{{ $jornada->id }}" {{ $punteo->jornada_id == $jornada->id ? 'selected' : '' }}>
                                    {{ $jornada->nombre }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-1">
                            ...
                            <select name="jugador_id" id="jugador" class="form-control select2 py-2 px-3 rounded-lg border-2 border-slate-300 mt-1 focus:outline-none focus:ring-2 focus:ring-slate-600 focus:border-transparent">
                                ...
                                <option value="{{ $jugador->id }}" data-image="{{ asset('imagen/' . $jugador->imagen) }}" {{ $punteo->jugador_id == $jugador->id ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1">
                            ...
                            <select name="tipo_tiro" id="tipoAccion" class="form-control select2 py-2 px-3 rounded-lg border-2 border-slate-300 mt-1 focus:outline-none focus:ring-2 focus:ring-slate-600 focus:border-transparent">
                                <option value="Tiro Libre" {{ $punteo->tipo_tiro == 'Tiro Libre' ? 'selected' : '' }}>Tiro Libre</option>
                                <option value="2 Puntos" {{ $punteo->tipo_tiro == '2 Puntos' ? 'selected' : '' }}>2 Puntos</option>
                                <option value="3 Puntos" {{ $punteo->tipo_tiro == '3 Puntos' ? 'selected' : '' }}>3 Puntos</option>
                                <option value="Otro" {{ Str::contains($punteo->tipo_tiro, 'Otro') ? 'selected' : '' }}>Otro, especifique</option>
                            </select>

                        </div>
                        <div class="grid grid-cols-1" style="display:grid;" id="otro_tipo_container">
                            ...
                            <input value="{{ $datoLimpio }}" name="otro_tipo" id="otro_tipo" class="py-2 px-3 rounded-lg border-2 border-slate-300 mt-1 focus:outline-none focus:ring-2 focus:ring-slate-600 focus:border-transparent" type="text" />
                        </div>
                        <div class="grid grid-cols-1">
                            ...
                            <input value="{{ $punteo->puntos_obtenidos }}" name="puntos_obtenidos" id="punteo_obtenido" class="py-2 px-3 rounded-lg border-2 border-slate-300 mt-1 focus:outline-none focus:ring-2 focus:ring-slate-600 focus:border-transparent" type="number" required />
                        </div>
...
<script>
    document.getElementById('tipoAccion').addEventListener('change', function() {
        ...
        var otroTipoContainer = document.getElementById('otro_tipo_container');
        ...
        //     punteoInput.value = '{{$punteo->puntos_obtenidos}}'; // Opcional: limpia el campo
        // } else {
